        ...
        {TABS.map(t => (
          <button
            ...
            className={`px-4 py-2 text-sm font-medium whitespace-nowrap rounded-t-lg transition-colors ${
              tab === t
                ? "bg-card border border-b-card border-border text-accent -mb-px"
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-3 items-center">
        <input
          ...
          className="border border-border rounded-lg px-3 py-1.5 text-sm bg-card text-fg focus:outline-none focus:ring-2 focus:ring-blue-400 w-56"
        />
        <select
          value={product} onChange={e => setProduct(e.target.value)}
          className="border border-border rounded-lg px-3 py-1.5 text-sm bg-card text-fg"
        >
        ...
        <span className="ml-auto text-xs text-fg-muted">{filtered.length} criteria</span>
        <button
      ...
      {/* Add form */}
      {showAdd && (
        <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800/50 rounded-xl p-4 space-y-3">
          <h3 className="text-sm font-semibold text-fg">New criterion</h3>
          <div className="grid grid-cols-2 gap-3">
            <Field label="ID (e.g. XDR-08)" value={newData.id ?? ""} onChange={v => setNewData(d => ({...d, id: v}))} />
            <div>
              <label className="block text-xs font-medium text-fg-dim mb-1">Product</label>
              <select value={newData.product ?? ""} onChange={e => setNewData(d => ({...d, product: e.target.value}))}
                className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-card text-fg">
          ...
          <div>
            <label className="block text-xs font-medium text-fg-dim mb-1">Edge Tags</label>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowAdd(false)} className="text-fg-dim text-sm px-4 py-2 rounded-lg hover:bg-card-alt">Cancel</button>
          </div>
        </div>
      )}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-card-alt border-b border-border">
              <th className="text-left px-4 py-2 text-xs font-semibold text-fg-muted uppercase w-20">ID</th>
              <th className="text-left px-4 py-2 text-xs font-semibold text-fg-muted uppercase w-24">Product</th>
              <th className="text-left px-4 py-2 text-xs font-semibold text-fg-muted uppercase">Requirement</th>
              <th className="text-left px-4 py-2 text-xs font-semibold text-fg-muted uppercase w-24">Tags</th>
          ...
          <tbody className="divide-y divide-border">
            {filtered.map(c => (
              <>
                <tr key={c.id} className={`hover:bg-card-alt transition-colors ${!c.active ? "opacity-50" : ""}`}>
                  <td className="px-4 py-2.5 font-mono text-xs text-fg-muted">{c.id}</td>
                  <td className="px-4 py-2.5">
                    <span className="text-xs bg-card-alt px-2 py-0.5 rounded font-medium text-fg-dim">{c.product}</span>
                  </td>
                  <td className="px-4 py-2.5 text-fg">{c.requirement}</td>
                  <td className="px-4 py-2.5">
                    <div className="flex flex-wrap gap-1">
                      {c.edgeTags.map(t => <span key={t} className="text-xs bg-blue-50 dark:bg-blue-950/30 text-blue-600 dark:text-blue-400 px-1.5 py-0.5 rounded">{t}</span>)}
                    </div>
                  </td>
                  <td className="px-4 py-2.5 text-right space-x-2">
                    ...
                    <button
                      ...
                      className={`text-xs font-medium ${c.active ? "text-fg-muted hover:text-red-600" : "text-emerald-600 hover:text-emerald-800"}`}
                    >
                ...
                {expandId === c.id && (
                  <tr key={`${c.id}-edit`}>
                    <td colSpan={5} className="px-4 py-4 bg-card-alt">
                      <div className="space-y-3">
                        ...
                        <div className="flex gap-2 pt-1">
                          ...
                          <button onClick={() => setExpandId(null)} className="text-fg-dim text-sm px-4 py-2 rounded-lg hover:bg-card-alt">Cancel</button>
                        </div>
                      </div>
                    </td>
                  </tr>
...
function OutcomesTab({
  ...
  return (
    <div className="space-y-4">
      ...
      {showAdd && (
        <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800/50 rounded-xl p-4 space-y-3">
          <h3 className="text-sm font-semibold text-fg">New outcome</h3>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-medium text-fg-dim mb-1">Color</label>
          ...
          <div>
            <label className="block text-xs font-medium text-fg-dim mb-1">Linked criteria ({(newData.criteriaIds ?? []).length})</label>
            <div className="max-h-32 overflow-y-auto border border-border rounded-lg p-2 space-y-1 bg-card">
              {allCriteria.filter(c => c.active).map(c => (
                <label key={c.id} className="flex items-center gap-2 text-xs cursor-pointer hover:bg-card-alt rounded px-1">
                  <input type="checkbox"
                  ...
                  <span className="text-fg-dim truncate">{c.requirement}</span>
                </label>
              ))}
            </div>
          </div>
          <div className="flex gap-2 pt-1">
            ...
            <button onClick={() => { setShowAdd(false); setAddError(""); }} className="text-fg-dim text-sm px-4 py-2 rounded-lg hover:bg-card-alt">Cancel</button>
          </div>
        </div>
      )}
      ...
      {outcomes.map(o => (
        <div key={o.id} className={`bg-card rounded-xl border border-border p-4 ${!o.active ? "opacity-50" : ""}`}>
          {editId === o.id ? (
            <div className="space-y-2">
              ...
              <div>
                <label className="block text-xs font-medium text-fg-dim mb-1">Linked criteria ({(editData.criteriaIds ?? []).length})</label>
                <div className="max-h-32 overflow-y-auto border border-border rounded-lg p-2 space-y-1 bg-card">
                  {allCriteria.filter(c => c.active).map(c => (
                    <label key={c.id} className="flex items-center gap-2 text-xs cursor-pointer hover:bg-card-alt rounded px-1">
                      <input type="checkbox"
              ...
              <div className="flex gap-2 pt-1">
                ...
                <button onClick={() => setEditId(null)} className="text-fg-dim text-xs px-3 py-1.5 rounded-lg hover:bg-card-alt">Cancel</button>
              </div>
            </div>
          ) : (
            <>
              <div className="flex items-center gap-2 mb-2">
                ...
                <div>
                  <p className="font-semibold text-fg text-sm">{o.title}</p>
                  <p className="text-xs text-fg-muted font-mono">{o.id}</p>
                </div>
              </div>
              <p className="text-xs text-fg-dim mb-3">{o.description}</p>
              <p className="text-xs text-fg-muted">{o.criteriaIds.length} criteria linked</p>
              <div className="flex gap-2 mt-3">
                ...
                <button
                  ...
                  className={`text-xs font-medium ${o.active ? "text-fg-muted hover:text-red-600" : "text-emerald-600 hover:text-emerald-800"}`}
                >
...
function PillarsTab({ pillars, setPillars }: { pillars: Pillar[]; setPillars: (p: Pillar[]) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {showAdd && (
        <div className="bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800/50 rounded-xl p-4 space-y-3">
          <h3 className="text-sm font-semibold text-fg">New pillar</h3>
          {addError && <p className="text-xs text-red-600 bg-red-50 border border-red-200 rounded px-3 py-1.5">{addError}</p>}
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-fg-dim mb-1">Tag (e.g. PLATFORM)</label>
              <input
                value={newPillar.tag}
                onChange={e => setNewPillar(d => ({...d, tag: e.target.value.toUpperCase()}))}
                placeholder="MYTAG"
                className="w-full px-3 py-2 border border-border rounded-lg text-sm font-mono bg-card text-fg focus:outline-none focus:ring-2 focus:ring-blue-400"
              />
            </div>
    ...
    <div className="bg-card rounded-xl border border-border overflow-hidden">
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-card-alt border-b border-border">
            <th className="text-left px-4 py-2 text-xs font-semibold text-fg-muted uppercase w-28">Tag</th>
            <th className="text-left px-4 py-2 text-xs font-semibold text-fg-muted uppercase">Name</th>
        ...
        <tbody className="divide-y divide-border">
          {pillars.map(p => (
            <>
              <tr key={p.tag} className={`hover:bg-card-alt ${!p.active ? "opacity-50" : ""}`}>
                <td className="px-4 py-2.5 font-mono text-xs text-fg-muted">{p.tag}</td>
                ...
                <td className="px-4 py-2.5 text-right space-x-2">
                  ...
                  <button
                    ...
                    className={`text-xs font-medium ${p.active ? "text-fg-muted hover:text-red-600" : "text-emerald-600"}`}
                  >{p.active ? "Deactivate" : "Reactivate"}</button>
                </td>
              </tr>
              {expandTag === p.tag && (
                <tr key={`${p.tag}-edit`}>
                  <td colSpan={3} className="px-4 py-4 bg-card-alt">
                    <div className="space-y-3 max-w-2xl">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button onClick={() => setExpandTag(null)} className="text-fg-dim text-sm px-4 py-2 rounded-lg hover:bg-card-alt">Cancel</button>
                      </div>
                    </div>
                  </td>
                </tr>
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        <input
          ...
          className="border border-border rounded-lg px-3 py-2 text-sm bg-card text-fg focus:outline-none focus:ring-2 focus:ring-blue-400 flex-1 max-w-md"
        />
        <button onClick={addItem}
          className="bg-[#0049BD] text-white text-sm font-semibold px-4 py-2 rounded-lg hover:bg-blue-700">
          Add
        </button>
        <label className="flex items-center gap-2 text-sm text-fg-dim cursor-pointer ml-auto">
          <input type="checkbox" checked={showInactive} onChange={e => setShowInactive(e.target.checked)} className="rounded" />
          Show inactive
        </label>
      </div>
      ...
      <div className="bg-card rounded-xl border border-border overflow-hidden">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-card-alt border-b border-border">
              <th className="text-left px-4 py-2 text-xs font-semibold text-fg-muted uppercase">Text</th>
              <th className="px-4 py-2 w-32" />
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {visible.map(item => (
              <tr key={item.id} className={`hover:bg-card-alt ${!item.active ? "opacity-50" : ""}`}>
                <td className="px-4 py-2.5">
                  {editId === item.id ? (
                    <input
                      ...
                      autoFocus
                      className="w-full border border-blue-300 rounded px-2 py-1 text-sm bg-card text-fg focus:outline-none focus:ring-2 focus:ring-blue-400"
                    />
                  ) : (
                ...
                <td className="px-4 py-2.5 text-right space-x-2">
                  {editId === item.id ? (
                    <>
                      ...
                      <button onClick={() => setEditId(null)} className="text-xs text-fg-muted">Cancel</button>
                    </>
                  ) : (
                    <>
                      ...
                      <button onClick={() => update(item.id, { active: !item.active })}
                        className={`text-xs font-medium ${item.active ? "text-fg-muted hover:text-red-600" : "text-emerald-600 hover:text-emerald-800"}`}>
            ...
            {visible.length === 0 && (
              <tr><td colSpan={2} className="px-4 py-8 text-center text-fg-muted text-sm">No items yet.</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
function Field({ label, value, onChange, type = "text" }: { label: string; value: string; onChange: (v: string) => void; type?: string }) {
  return (
    <div>
      <label className="block text-xs font-medium text-fg-dim mb-1">{label}</label>
  ...
  return (
    <div>
      ...
      <textarea rows={rows} value={value} onChange={e => onChange(e.target.value)}
        className="w-full px-3 py-2 border border-border rounded-lg text-sm bg-card text-fg focus:outline-none focus:ring-2 focus:ring-blue-400 resize-y" />
    </div>
  );
}
